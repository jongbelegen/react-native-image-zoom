...
import type { GestureStateChangeEvent, PanGestureHandlerEventPayload, PinchGestureHandlerEventPayload, TapGestureHandlerEventPayload } from 'react-native-gesture-handler';
import { AnimatableValue } from 'react-native-reanimated';
export type OnPinchStartCallback = (event: GestureStateChangeEvent<PinchGestureHandlerEventPayload>) => void;
export type OnPinchEndCallback = (event: GestureStateChangeEvent<PinchGestureHandlerEventPayload>, success: boolean) => void;
export type OnPanStartCallback = (event: GestureStateChangeEvent<PanGestureHandlerEventPayload>) => void;
export type OnPanEndCallback = (event: GestureStateChangeEvent<PanGestureHandlerEventPayload>, success: boolean) => void;
export type OnSingleTapCallback = (event: GestureStateChangeEvent<TapGestureHandlerEventPayload>) => void;
export declare enum ZOOM_TYPE {
    ZOOM_IN = "ZOOM_IN",
    ZOOM_OUT = "ZOOM_OUT"
}
export type OnDoubleTapCallback = (zoomType: ZOOM_TYPE) => void;
export declare enum ANIMATION_VALUE {
    SCALE = "SCALE",
    FOCAL_X = "FOCAL_X",
    FOCAL_Y = "FOCAL_Y",
...
export type OnResetAnimationEndCallback = (finished?: boolean, values?: Record<ANIMATION_VALUE, {
    finished?: boolean;
...
export type ImageZoomUseGesturesProps = Pick<ImageZoomLayoutState, 'width' | 'height' | 'center'> & Pick<ImageZoomProps, 'minScale' | 'maxScale' | 'doubleTapScale' | 'minPanPointers' | 'maxPanPointers' | 'isPanEnabled' | 'isPinchEnabled' | 'isSingleTapEnabled' | 'isDoubleTapEnabled' | 'onInteractionStart' | 'onInteractionEnd' | 'onPinchStart' | 'onPinchEnd' | 'onPanStart' | 'onPanEnd' | 'onSingleTap' | 'onDoubleTap' | 'onResetAnimationEnd'>;
